...
def get_analytics_summary(business_id, period):
    ...
    try:
        ...
        first_visit_rows = cursor.execute(
            """
            SELECT
                client_id,
    ...
    first_visit_map = {
        row["client_id"]: row["first_booking_date"]
    ...
    total_revenue = 0.0
    no_show_lost_raw = 0.0
    cancelled_lost_raw = 0.0
    ...
    for row in rows:
        ...
        if status == "completed":
            ...
            if service_id:
                ...
                service_stats[service_id]["revenue"] += price
            ...
            if employee_id:
                ...
                employee_stats[employee_id]["bookings"] += 1
            ...
            no_show_count += 1
            no_show_lost_raw += price

        elif status == "cancelled":
            cancelled_count += 1
            cancelled_lost_raw += price

    measured_visits = completed_bookings + no_show_count + cancelled_count
    no_show_rate = round((no_show_count / measured_visits) * 100, 1) if measured_visits else 0
    average_visit_value = round(total_revenue / completed_bookings, 2) if completed_bookings else 0

    new_clients_count = 0

    for client_id in active_client_ids:
        first_date = first_visit_map.get(client_id)

        if first_date and start_date <= first_date <= end_date:
            new_clients_count += 1

    returning_clients_count = sum(
        1 for count in client_visit_counts.values()
        if count > 1
    )

    top_services_revenue_raw = sorted(
    ...
    top_services_revenue = [
        ...
        for item in top_services_revenue_raw
    ]

    employee_revenue_raw = sorted(
    ...
    employee_revenue = [
        ...
        for item in employee_revenue_raw
    ]

    revenue_by_hour_raw = sorted(
        hour_stats.values(),
        key=lambda item: item["revenue"],
        reverse=True
    )[:6]
    ...
    revenue_by_hour = [
        ...
        for item in revenue_by_hour_raw
    ]

    top_service = top_services_revenue_raw[0] if top_services_revenue_raw else None

    top_employee = None
    if employee_revenue_raw:
        top_employee = employee_revenue_raw[0]

    best_day_name = "—"
    best_day_revenue_raw = 0.0

    if weekday_revenue_map:
        best_day_name, best_day_revenue_raw = max(
            weekday_revenue_map.items(),
            key=lambda item: item[1]
        )

        if best_day_revenue_raw <= 0:
            best_day_name = "—"

    lost_revenue_raw = no_show_lost_raw + cancelled_lost_raw
    ...
    return {
        ...
        "average_visit_value": format_pln(average_visit_value),
        ...
        "top_service_name": top_service["name"] if top_service else "—",
        "top_service_count": top_service["count"] if top_service else 0,
        "top_service_revenue": format_pln(top_service["revenue"]) if top_service else format_pln(0),

        "top_employee_name": top_employee["name"] if top_employee else "—",
        "top_employee_bookings": top_employee["bookings"] if top_employee else 0,

        "best_day_name": best_day_name,
        "best_day_revenue": format_pln(best_day_revenue_raw),

        "no_show_lost": format_pln(no_show_lost_raw),
        "cancelled_lost": format_pln(cancelled_lost_raw),
        "lost_revenue": format_pln(lost_revenue_raw),
...
def add_service():
    ...
    if not valid_employee_ids:
        if is_ajax:
            return jsonify({"success": False, "message": "Wybierz co najmniej jednego pracownika dla usługi."}), 400
    ...
    try:
        ...
        if is_ajax:
            return jsonify({"success": False, "message": "Nie udało się dodać usługi."}), 500
...
def update_archived_booking_status(booking_id):
    ...
    try:
        ...
        if new_status == "completed":
            flash("Wizyta oznaczona jako zrealizowana.", "success")
        else:
            flash("Wizyta oznaczona jako nieobecność. Klient dodany do czarnej listy.", "success")
...
def create_manual_booking():
    ...
    try:
        ...
        if service_mode == "custom" and created_custom_service and final_service_id:
            try:
                cursor.execute(
                    "DELETE FROM service_employees WHERE service_id = ?",
                    (final_service_id,)
