...
def upload_marketing_template_image(file_storage, business_id):
    if not file_storage or not file_storage.filename:
        return None, None

    result = cloudinary.uploader.upload(
        file_storage,
        folder=f"alifio/marketing/{business_id}",
        resource_type="image"
    )
...
def ai_text_skips_note(value):
    ...
    return any(phrase == text or phrase in text for phrase in phrases)    
...
def ai_services_reply(services):
    ...
    for service in services or []:
        ...
        if duration:
            label_parts.append(f"{duration} min")
        ...
        suggestions.append({
            "label": label,
            "message": f"__service_id:{service_id}",
            "display": f"Wybieram usługę: {name}"
        })

    reply = "Wybierz usługę, którą chcesz zarezerwować."
...
def ai_employees_reply(service):
    ...
    for employee in service.get("employees", []) or []:
        name = employee.get("name") or ""

        suggestions.append({
            "label": name,
            "message": f"Wybieram pracownika: {name}"
        })

    reply = f"Kto ma wykonać usługę „{service.get('name', 'wybraną usługę')}”?"
...
def ai_format_slots_reply(service, employee, date_iso, slots):
    ...
    for slot in slots:
        suggestions.append({
            "label": slot,
            "message": f"Wybieram godzinę: {slot}"
        })

    if not slots:
        reply = (
            f"Nie ma wolnych godzin dla usługi „{service.get('name', 'wybrana usługa')}” "
            f"u {employee.get('name', 'wybranego specjalisty')} na dzień {date_display}."
        )
    else:
        reply = (
            f"Dostępne godziny dla usługi „{service.get('name', 'wybrana usługa')}” "
            f"u {employee.get('name', 'wybranego specjalisty')} na dzień {date_display}:"
        )
...
def ai_create_booking_from_context(context):
    ...
    client_note = (context.get("client_note") or "").strip()
    ...
    client_id = get_or_create_client(
        ...
        consent_source="automatic_booking_assistant",
    ...
    booking_note_parts = [
        "Rezerwacja utworzona przez automatycznego asystenta rezerwacji."
    ]

    if client_note:
        booking_note_parts.append(f"Wiadomość klienta dla salonu: {client_note}")
    ...
    try:
        ...
        print("Błąd wysyłki maili wewnętrznych automatic booking assistant:", e)
    ...
    if client_email:
        try:
            ...
            print("Błąd wysyłki maila automatic booking assistant:", e)
...
def ai_chat():
    ...
    def make_actions(items):
        actions = []

        for item in items or []:
            if isinstance(item, dict):
                label = item.get("label") or item.get("text") or item.get("name") or item.get("message")
                message_value = item.get("message") or label
                display_value = item.get("display") or label
            else:
                label = str(item)
    ...
    def json_response(reply, context=None, slots=None, suggestions=None, actions=None, status_code=200):
        return jsonify({
            "success": status_code < 400,
            "reply": reply,
            "slots": slots or [],
            "suggestions": suggestions or [],
            "actions": actions or make_actions(suggestions or []),
            "pending_context": context or {}
        }), status_code

    def invalid_date_response(current_context):
        reply = (
            "Nie udało się rozpoznać daty wizyty.\n\n"
            "Podaj datę np. DD.MM albo DD.MM.RRRR."
        )

        current_context["stage"] = "date"
        session["alifio_ai_pending_context"] = current_context
    ...
    def no_slots_date_response(current_context, current_service, current_employee):
        current_context.pop("date_iso", None)
        current_context["stage"] = "date"

        reply = (
            f"Nie ma wolnych godzin dla usługi „{current_service['name']}” "
            f"u {current_employee['name']} w wybranym dniu.\n\n"
            "Podaj inny dzień wizyty."
        )
    ...
    def is_allowed_date_input(value):
        raw_value = (value or "").strip().lower()
        normalized_value = ai_normalize_text(value)
        ...
        return bool(date_pattern.match(raw_value)) or normalized_value in quick_words

    if not message:
        return json_response(
            "Napisz wiadomość, a pomogę wybrać usługę i termin.",
            status_code=400
        )

    services = ai_get_services_catalog()

    def stage_help_response(current_stage, current_context, current_service=None, current_employee=None):
        if current_stage == "service":
            ...
            return json_response(
                ...
                actions=make_actions(suggestions)
            )

        if current_stage == "employee" and current_service:
            reply, suggestions = ai_employees_reply(current_service)
        ...
        if current_stage == "date":
            return json_response(
                reply=(
                    "Teraz podaj dzień wizyty.\n\n"
                    "Podaj datę np. DD.MM albo DD.MM.RRRR."
                ),
                context=current_context
            )

        if current_stage == "time":
            if current_service and current_employee and current_context.get("date_iso"):
                try:
                    available_slots = get_available_slots_for_day(
                        current_context["service_id"],
                        current_context["employee_id"],
                        current_context["date_iso"]
                    )

                    slots = available_slots

                    if not slots:
                        return no_slots_date_response(
                            current_context,
                            current_service,
                            current_employee
                        )

                    reply, suggestions = ai_format_slots_reply(
                        current_service,
                        current_employee,
                        current_context["date_iso"],
                        slots
                    ...
                    print("Błąd pomocy etapu time automatic assistant:", e)

            return json_response(
                reply="Teraz wybierz jedną z dostępnych godzin albo wpisz godzinę ręcznie, np. 14:30.",
                context=current_context
            )

        if current_stage == "client_name":
            return json_response(
                reply="Proszę podać imię i nazwisko potrzebne do zapisania rezerwacji.",
                context=current_context
            )

        if current_stage == "client_phone":
            return json_response(
                reply="Teraz podaj numer telefonu do rezerwacji.",
                context=current_context
            )

        if current_stage == "client_email":
            return json_response(
                reply=(
                    "Teraz podaj adres e-mail albo kliknij „bez maila”.\n\n"
                    "Podanie adresu e-mail pozwoli na otrzymanie potwierdzenia rezerwacji i przypomnienia o wizycie."
                ),
                context=current_context,
                suggestions=["bez maila"],
                actions=make_actions(["bez maila"])
            )

        if current_stage == "client_note":
            return json_response(
                reply=(
                    "Czy chcesz dodać wiadomość dla salonu?\n\n"
                    "Możesz wpisać np. „Proszę o kontakt przed wizytą” albo wybrać „Bez notatki”."
                ),
                context=current_context,
                suggestions=["Bez notatki"],
                actions=make_actions(["Bez notatki"])
            )

        if current_stage == "privacy":
            return json_response(
                reply="Teraz potrzebuję potwierdzenia akceptacji polityki prywatności.",
                context=current_context,
                suggestions=["Akceptuję politykę prywatności"],
                actions=make_actions(["Akceptuję politykę prywatności"])
            )

        if current_stage == "marketing":
            return json_response(
                reply="Zgoda marketingowa jest dobrowolna. Wybierz odpowiedź.",
                context=current_context,
                suggestions=["Tak", "Nie"],
                actions=make_actions(["Tak", "Nie"])
            )

        return json_response(
            reply="Pomogę Ci przejść przez rezerwację krok po kroku.",
            context=current_context
        )

    start_phrases = [
        "chce rozpoczac rezerwacje",
        "chce rozpocząć rezerwację",
        "chce zarezerwowac termin",
        "chce zarezerwować termin",
        "rozpocznij rezerwacje",
    ...
    if any(phrase == normalized or phrase in normalized for phrase in continue_phrases):
        return stage_help_response(
            stage,
            context,
            selected_service,
            selected_employee
    ...
    if not selected_service:
        ...
        if message.startswith("__service_id:"):
            try:
                selected_service_id = int(message.replace("__service_id:", "").strip())
                found_service = ai_find_service_by_id(services, selected_service_id)
            except Exception:
                found_service = None
    ...
    if not context.get("date_iso"):
        if not is_allowed_date_input(message):
            return invalid_date_response(context)
        ...
        if date_iso:
            ...
            try:
                ...
                print("Błąd sprawdzania slotów automatic assistant po wyborze daty:", e)

                reply = "Nie udało się sprawdzić godzin. Podaj datę jeszcze raz w formacie DD.MM albo DD.MM.RRRR."
    ...
    try:
        ...
        print("Błąd sprawdzania slotów automatic assistant:", e)
    ...
    if not context.get("client_phone"):
        ...
        if stage == "client_phone" and phone:
            ...
            reply = "Dziękuję. Proszę podać numer telefonu potrzebny do zapisania rezerwacji."
    ...
    if not context.get("client_email") and not context.get("client_email_skipped"):
        ...
        if stage == "client_email" and email:
            ...
            reply = (
                ...
                "Jeśli nie chcesz podawać e-maila, kliknij lub napisz: „bez maila”.\n\n"
                "Podanie adresu e-mail pozwoli na otrzymanie potwierdzenia rezerwacji i przypomnienia o wizycie."
            )
    ...
    # =========================================================
    # ETAP: WIADOMOŚĆ DLA SALONU / NOTATKA
    # =========================================================

    if not context.get("client_note_answered"):
        if stage == "client_note":
            if ai_text_skips_note(message):
                context["client_note"] = ""
            else:
                context["client_note"] = message.strip()

            context["client_note_answered"] = True
    ...
    if not context.get("privacy_consent_confirmed"):
        if stage == "privacy" and ai_text_confirms_privacy(message):
            ...
            if privacy_url:
                reply = (
                    "W celu zapisania rezerwacji, potrzebuję potwierdzenia akceptacji Polityki Prywatności.\n\n"
                    f"Polityka prywatności: {privacy_url}\n\n"
                    "Kliknij albo napisz: „Akceptuję politykę prywatności”.\n\n"
                    "Jeżeli nie wyrażasz zgody na przetwarzanie danych osobowych, rezerwacja nie będzie mogła zostać zapisana."
                )
            else:
                reply = (
                    "Kliknij albo napisz: „Akceptuję politykę prywatności”.\n\n"
    ...
    if not context.get("marketing_consent_answered"):
        ...
        if stage == "marketing" and marketing_result is not None:
            ...
            reply = "Czy chcesz otrzymywać informacje marketingowe i promocyjne?"
    ...
    if result.get("success"):
        ...
        client_note = (context.get("client_note") or "").strip()
        client_note_line = f"Wiadomość dla salonu: {client_note}\n" if client_note else ""
        ...
        reply = (
            ...
            f"{client_note_line}\n"
            "Jeżeli adres e-mail został podany, potwierdzenie rezerwacji oraz przypomnienie o terminie zostaną wysłane automatycznie.\n\n"
            "Jeżeli konieczna będzie zmiana terminu wizyty, prosimy o anulowanie obecnej rezerwacji i dokonanie nowej. "
            "Instrukcje anulowania znajdują się w wiadomości e-mail z potwierdzeniem wizyty.\n\n"
            "W razie potrzeby prosimy o kontakt z punktem usługowym.\n\n"
            "Pozdrawiamy!"
        )
...
def update_settings():
    ...
    booking_page_url = (request.form.get("booking_page_url") or "").strip()
    ...
    booking_page_url = normalize_url(booking_page_url)
...
@app.route("/admin/bookings/archive/mark-confirmed-completed", methods=["POST"])
@client_admin_required
def mark_all_confirmed_archived_completed():
    business_id = session.get("business_id", 1)
    ...
    try:
        ...
        flash(f"Oznaczono jako zrealizowane: {updated_count} wizyt.", "success")

    except Exception as e:
        conn.rollback()
        print("mark_all_confirmed_archived_completed error:", e)
        flash("Nie udało się oznaczyć wizyt jako zrealizowane.", "error")

    finally:
        conn.close()

    return redirect(url_for("admin_dashboard") + "#archive-section")
...
def admin_settings():
    ...
    try:
        ...
        marketing_templates = cursor.execute(
            """
            SELECT
                id,
                business_id,
    ...
    employee_time_off_map = {}
    employee_schedule_exceptions_map = {}

    admin_login_email = ""
    if current_user and current_user["email"]:
        admin_login_email = current_user["email"]
...
# =========================================================
# SAVE MARKETING TEMPLATE
# =========================================================

@app.route("/admin/marketing/save", methods=["POST"])
@client_admin_required
def save_marketing_template():
    ...
    audience_type = (request.form.get("audience_type") or "all_marketing").strip()
    discount_value = (request.form.get("discount_value") or "").strip()
    service_name_value = (request.form.get("service_name_value") or "").strip()
    offer_deadline = (request.form.get("offer_deadline") or "").strip()
    cta_url = (request.form.get("cta_url") or "").strip()

    remove_image = (request.form.get("remove_image") or "0").strip() == "1"

    if not template_key:
        flash("Nie wybrano szablonu marketingowego.", "error")
        return redirect(url_for("admin_settings") + "#marketing")

    if not template_name:
        template_name = subject or "Szablon marketingowy"
    ...
    try:
        ...
        cursor.execute(
            ...
            LIMIT 1
            """,
            (business_id, template_key)
        )

        existing = cursor.fetchone()
        current_image_path = existing["image_path"] if existing else None
        ...
        if image and image.filename:
            ...
            if extension not in allowed_extensions:
                flash("Dozwolone formaty zdjęcia: PNG, JPG, JPEG, WEBP.", "error")
                return redirect(url_for("admin_settings") + "#marketing")

            upload_dir = os.path.join(app.root_path, "static", "uploads", "marketing")
            os.makedirs(upload_dir, exist_ok=True)

            safe_name = f"marketing_{business_id}_{template_key}_{int(time.time())}{extension}"
            save_path = os.path.join(upload_dir, safe_name)

            image.save(save_path)
            image_path = f"uploads/marketing/{safe_name}"
        ...
        cursor.execute(
            ...
            DO UPDATE SET
                ...
                service_name_value = excluded.service_name_value,
        ...
        conn.commit()
        flash("Szablon marketingowy został zapisany.", "success")

    except Exception as e:
        conn.rollback()
        print("Błąd save_marketing_template:", e)
        flash("Nie udało się zapisać szablonu marketingowego.", "error")
...
# =========================================================
# MARKETING CAMPAIGNS — RECIPIENTS / PREVIEW / SEND
# =========================================================

def marketing_normalize_audience(value):
    value = (value or "").strip()
    ...
    return value if value in allowed else "all_marketing"


def marketing_replace_placeholders(text, context):
    text = text or ""

    replacements = {
        "{{client_name}}": context.get("client_name") or "Kliencie",
        "{{business_name}}": context.get("business_name") or "",
        "{{booking_link}}": context.get("booking_link") or "",
        "{{discount}}": context.get("discount") or "",
        "{{service_name}}": context.get("service_name") or "",
        "{{offer_deadline}}": context.get("offer_deadline") or "",
    }
...
def get_marketing_recipients(business_id, audience_type):
    audience_type = marketing_normalize_audience(audience_type)
    today = datetime.now().date()
    ...
    try:
        rows = cursor.execute(
            ...
            SELECT
                ...
                MAX(
                    CASE
                        WHEN COALESCE(b.archived, 0) = 0
                         AND b.booking_date >= date('now')
                         AND LOWER(COALESCE(b.status, '')) NOT IN ('cancelled', 'no_show', 'completed')
                        THEN b.booking_date
                        ELSE NULL
                    END
    ...
    for row in rows:
        ...
        marketing_consent = int(row["marketing_consent"] or 0)
        last_completed_visit = row["last_completed_visit"]
        next_booking_date = row["next_booking_date"]

        include = False

        if audience_type == "all_marketing":
            include = True

        elif audience_type in ("inactive_30", "inactive_60", "inactive_90"):
            days_limit = {
                "inactive_30": 30,
                "inactive_60": 60,
                "inactive_90": 90,
            ...
            if last_completed_visit and not next_booking_date:
                try:
                    last_date = datetime.strptime(last_completed_visit, "%Y-%m-%d").date()
                    include = (today - last_date).days >= days_limit
                except Exception:
                    include = False

        elif audience_type == "after_visit":
            if last_completed_visit:
                try:
                    last_date = datetime.strptime(last_completed_visit, "%Y-%m-%d").date()
                    include = 0 <= (today - last_date).days <= 7
                except Exception:
                    include = False
        ...
        recipient = {
            "id": row["id"],
            "name": row["full_name"] or "Klient bez nazwy",
            "email": email,
            "phone": row["phone"] or "",
            "last_completed_visit": last_completed_visit or "",
            "next_booking_date": next_booking_date or "",
            "marketing_consent": marketing_consent,
            "ready": bool(email and marketing_consent == 1),
        }
    ...
    return {
        ...
        "no_consent_count": no_consent,
        "skipped_count": no_email + no_consent,
        "recipients": matched,
        "ready_recipients": ready,
    }


def build_marketing_email_html(subject, body, cta_text, cta_url, image_url, company_context):
    safe_subject = subject or ""
    safe_body = (body or "").replace("\n", "<br>")
    safe_cta = cta_text or "Zarezerwuj termin"
    safe_cta_url = cta_url or "#"

    image_html = ""
    if image_url:
        image_html = f"""
        <div style="padding:0 24px 24px 24px;">
          <img src="{image_url}" alt="" style="display:block; width:100%; max-width:720px; height:auto; border-radius:14px;">
        </div>
        """

    footer_rows = []

    if company_context.get("company_address"):
        footer_rows.append(company_context["company_address"])

    if company_context.get("contact_phone"):
        footer_rows.append(f"Tel.: {company_context['contact_phone']}")

    if company_context.get("contact_email"):
        footer_rows.append(f"E-mail: {company_context['contact_email']}")

    if company_context.get("privacy_policy_url"):
        footer_rows.append(
            f'<a href="{company_context["privacy_policy_url"]}" style="color:#6b7280; text-decoration:none;">Polityka prywatności</a>'
        )

    if company_context.get("withdraw_marketing_consent_url"):
        footer_rows.append(
            f'<a href="{company_context["withdraw_marketing_consent_url"]}" style="color:#6b7280; text-decoration:none;">Wycofaj zgodę marketingową</a>'
        )

    footer_html = "<br>".join(footer_rows)
    ...
    <div style="margin:0; padding:0; background:#f4f5f7;">
      <div style="max-width:760px; margin:0 auto; padding:28px 14px;">
        <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:18px; overflow:hidden;">
          <div style="padding:22px 24px; background:#f9fafb; border-bottom:1px solid #e5e7eb;">
            <div style="font-family:Arial, sans-serif; font-size:18px; line-height:1.45; color:#111827; font-weight:600;">
              {safe_subject}
            </div>
          </div>

          <div style="padding:24px; font-family:Arial, sans-serif; font-size:15px; line-height:1.8; color:#111827;">
            {safe_body}
          </div>

          <div style="padding:0 24px 24px 24px; text-align:center;">
            <a href="{safe_cta_url}" style="display:inline-block; padding:14px 24px; border-radius:999px; background:#111827; color:#ffffff; text-decoration:none; font-family:Arial, sans-serif; font-size:14px;">
              {safe_cta}
            </a>
          </div>
          ...
          <div style="padding:18px 24px; border-top:1px solid #e5e7eb; background:#f9fafb; font-family:Arial, sans-serif; font-size:12px; line-height:1.7; color:#6b7280;">
            {footer_html}
          </div>
        </div>
      </div>
...
def build_marketing_email_text(body, company_context):
    footer = []

    if company_context.get("company_name"):
        footer.append(company_context["company_name"])

    if company_context.get("company_address"):
        footer.append(company_context["company_address"])

    if company_context.get("contact_phone"):
        footer.append(f"Tel.: {company_context['contact_phone']}")

    if company_context.get("contact_email"):
        footer.append(f"E-mail: {company_context['contact_email']}")

    if company_context.get("privacy_policy_url"):
        footer.append(f"Polityka prywatności: {company_context['privacy_policy_url']}")

    if company_context.get("withdraw_marketing_consent_url"):
        footer.append(f"Wycofaj zgodę marketingową: {company_context['withdraw_marketing_consent_url']}")

    return (body or "").strip() + "\n\n" + "\n".join(footer)


@app.route("/admin/marketing/recipients")
@client_admin_required
def marketing_recipients_preview():
    business_id = session.get("business_id", 1)
    audience_type = marketing_normalize_audience(request.args.get("audience_type"))

    result = get_marketing_recipients(business_id, audience_type)
    ...
    return jsonify({
        ...
        "no_consent_count": result["no_consent_count"],
        "skipped_count": result["skipped_count"],
        "recipients": result["recipients"][:100],
    })


@app.route("/admin/marketing/test", methods=["POST"])
@client_admin_required
def send_marketing_test():
    business_id = session.get("business_id", 1)
    settings = get_settings(business_id)

    admin_email = session.get("admin_email") or ""
    fallback_email = settings["contact_email"] if settings and "contact_email" in settings.keys() else ""
    test_email = admin_email or fallback_email

    if not test_email:
        return jsonify({
            "success": False,
            "message": "Brak adresu e-mail administratora do wysyłki testowej."
        }), 400
    ...
    if not subject or not body:
        return jsonify({
            "success": False,
            "message": "Uzupełnij temat i treść wiadomości."
        }), 400

    company_name = settings["company_name"] if settings and settings["company_name"] else "Twoja firma"
    booking_link = cta_url or (settings["booking_page_url"] if settings and "booking_page_url" in settings.keys() else "")
    ...
    final_subject = marketing_replace_placeholders(subject, placeholder_context)
    final_body = marketing_replace_placeholders(body, placeholder_context)

    company_context = {
        "company_name": company_name,
        "company_address": settings["company_address"] if settings and "company_address" in settings.keys() else "",
        "contact_phone": settings["contact_phone"] if settings and "contact_phone" in settings.keys() else "",
        "contact_email": settings["contact_email"] if settings and "contact_email" in settings.keys() else "",
        "privacy_policy_url": settings["privacy_policy_url"] if settings and "privacy_policy_url" in settings.keys() else "",
        "withdraw_marketing_consent_url": "",
    }
    ...
    text_body = build_marketing_email_text(final_body, company_context)

    try:
        send_email_message(
            to_email=test_email,
            subject="[TEST] " + final_subject,
            html_body=html_body,
            text_body=text_body,
        )
        ...
        return jsonify({
            "success": True,
            "message": f"Wysłano test na adres: {test_email}"
        })

    except Exception as e:
        print("Błąd send_marketing_test:", e)
        return jsonify({
            "success": False,
            "message": "Nie udało się wysłać testu."
        }), 500
...
def send_marketing_campaign():
    ...
    audience_type = marketing_normalize_audience(request.form.get("audience_type"))
    ...
    recipients_data = get_marketing_recipients(business_id, audience_type)
    recipients = recipients_data["ready_recipients"]

    if not recipients:
        return jsonify({
            "success": False,
            "message": "Brak odbiorców gotowych do wysyłki."
        }), 400
    ...
    for recipient in recipients:
        try:
            token = get_or_assign_client_action_token(recipient["id"])

            withdraw_url = ""
            if token:
                links = attach_client_action_links({}, token)
                withdraw_url = links.get("withdraw_marketing_consent_url", "")
            ...
            company_context = {
                ...
                "contact_email": settings["contact_email"] if settings and "contact_email" in settings.keys() else "",
                "privacy_policy_url": settings["privacy_policy_url"] if settings and "privacy_policy_url" in settings.keys() else "",
                "withdraw_marketing_consent_url": withdraw_url,
            }
            ...
            failed_count += 1
            print(f"Błąd wysyłki marketingowej do client_id={recipient['id']}:", e)

    return jsonify({
        "success": True,
        "message": f"Wysłano kampanię do {sent_count} klientów. Błędy: {failed_count}.",
        "sent_count": sent_count,
        "failed_count": failed_count,
        "matched_count": recipients_data["matched_count"],
        "ready_count": recipients_data["ready_count"],
        "skipped_count": recipients_data["skipped_count"],
    })
