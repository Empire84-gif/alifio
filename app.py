...
def get_analytics_summary(business_id, period):
    ...
    weekday_labels = ["Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela"]
    ...
    try:
        ...
        rows = cursor.execute(f"""
            SELECT
                ...
                {service_duration_sql} AS service_duration_minutes,
    ...
    def get_status_display(status):
        labels = {
            ...
            "cancelled": ("Anulowana", "is-warning"),
            "no_show": ("Nieobecność", "is-danger"),
    ...
    for row in rows:
        ...
        if service_id:
            service_stats.setdefault(service_id, {
                "id": service_id,
            ...
            service_stats[service_id]["visits_count"] += 1

            if status == "completed":
                service_stats[service_id]["completed_count"] += 1
                service_stats[service_id]["count"] += 1
                service_stats[service_id]["revenue"] += price
            elif status == "cancelled":
                service_stats[service_id]["cancelled_count"] += 1
                service_stats[service_id]["lost_revenue"] += price
            elif status == "no_show":
                service_stats[service_id]["no_show_count"] += 1
                service_stats[service_id]["lost_revenue"] += price
    ...
    top_employees = sorted(employee_stats.values(), key=lambda x: x["revenue"], reverse=True)[:8]
    ...
    for client in client_stats.values():
        ...
        if last_visit_date:
            try:
                last_visit_display = datetime.strptime(
                    last_visit_date,
                    "%Y-%m-%d"
                ).strftime("%d.%m.%Y")
            except Exception:
    ...
    return {
        ...
        "top_employee_name": top_employees[0]["name"] if top_employees else "—",
        "top_employee_bookings": top_employees[0]["bookings"] if top_employees else 0,
...
def update_archived_booking_status(booking_id):
    ...
    try:
        ...
        if new_status == "completed":
            ...
            flash("Wizyta oznaczona jako nieobecność. <br>Klient dodany do czarnej listy.", "success")
