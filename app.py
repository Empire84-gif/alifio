...
def send_booking_status_changed_emails(notification_ctx, new_status):
    ...
    if normalized_status == "cancelled":
        ...
        client_body = dedent(f"""
            ...
            - Pracownik: {notification_ctx['employee_name']}
            - Data: {notification_ctx['booking_date_pl']}
            - Godzina: {notification_ctx['booking_time']}

            W razie potrzeby skontaktuj się bezpośrednio z salonem.
        ...
        client_body = dedent(f"""
            ...
            informujemy, że Twoja rezerwacja została potwierdzona.
        ...
        internal_body = dedent(f"""
            W systemie potwierdzono rezerwację.
...
def send_waitlist_promoted_emails(notification_ctx):
    ...
    client_body = dedent(f"""
        ...
        mamy dobrą wiadomość — znaleziono dostępny termin i Twoje zgłoszenie z listy oczekujących zostało przeniesione do aktywnej rezerwacji.
    ...
    try:
        cursor.execute("""
            SELECT
                ...
                bs.company_name,
                bs.company_email AS salon_email,
...
def update_booking_status(booking_id):
    ...
    if (
        (previous_status == "confirmed" and new_status == "cancelled") or
        (previous_status == "cancelled" and new_status == "confirmed")
    ):
        try:
...
def update_booking(booking_id):
    ...
    if (
        (previous_status == "confirmed" and status == "cancelled") or
        (previous_status == "cancelled" and status == "confirmed")
    ):
        try:
            booking_data = fetch_booking_notification_data(booking_id)
            if booking_data:
