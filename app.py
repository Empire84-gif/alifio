...
def calculate_client_status(total_bookings, last_booking_at):
    from datetime import datetime, timedelta
    ...
    # sprawdzenie nieaktywności (90 dni)
    if last_booking_at:
        try:
            last_date = datetime.strptime(last_booking_at, "%Y-%m-%d %H:%M:%S")
            if last_date < datetime.now() - timedelta(days=90):
                return "inactive"
        except:
            pass
