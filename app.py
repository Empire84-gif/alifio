...
def ai_get_terms_url():
    settings = get_settings()

    if settings and "terms_url" in settings.keys():
        return (settings["terms_url"] or "").strip()
...
def ai_text_confirms_privacy(value):
    ...
    positive_phrases = [
        ...
        "potwierdzam polityke prywatnosci",
        "akceptuje polityke",
        "zgadzam sie na polityke",
        "akceptuje",
        "zgadzam sie",
        "potwierdzam",
        "tak akceptuje",
    ...
    return any(ai_normalize_text(phrase) in text for phrase in positive_phrases)
...
def ai_text_confirms_marketing(value):
    ...
    negative_phrases = [
        "nie zgadzam sie na marketing",
        "nie chce marketingu",
        "nie chce otrzymywac marketingu",
        "bez marketingu",
        "nie",
        "odmawiam",
    ]
    ...
    positive_phrases = [
        "zgadzam sie na marketing",
        "akceptuje marketing",
        "tak marketing",
        "tak zgadzam sie",
        "chce otrzymywac informacje marketingowe",
        "chce marketing",
        "tak",
    ]

    if any(ai_normalize_text(phrase) == text or ai_normalize_text(phrase) in text for phrase in negative_phrases):
        return False

    if any(ai_normalize_text(phrase) == text or ai_normalize_text(phrase) in text for phrase in positive_phrases):
        return True
...
def ai_parse_openai_output(response_json):
...
def ai_call_openai(message, conversation, services):
    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    model = os.getenv("OPENAI_MODEL", "gpt-4.1-mini").strip()

    if not api_key:
        return {
            "reply": "Konfiguracja asystenta nie jest jeszcze zakończona. Brakuje klucza API.",
            ...
            "client_phone": "",
            "client_email": "",
            "should_check_slots": False,
            "should_create_booking": False
        }

    today = datetime.now().strftime("%Y-%m-%d")
    privacy_policy_url = ai_get_privacy_policy_url()
    terms_url = ai_get_terms_url()
    ...
    system_prompt = f"""
Jesteś ALIFIO AI Concierge — premium asystent rezerwacji dla eleganckiego systemu Alifio.

Dzisiejsza data: {today}.
Link do polityki prywatności: {privacy_policy_url or "brak linku w ustawieniach"}.
Link do regulaminu: {terms_url or "brak linku w ustawieniach"}.

Język: polski.
Styl: spokojny, profesjonalny, konkretny, premium. Bez przesadnych emoji. Bez chaosu.

Twoim zadaniem jest:
1. zrozumieć intencję klienta,
2. rozpoznać usługę, datę, preferowany czas, wybraną godzinę, pracownika i dane klienta,
3. jeśli brakuje danych — zadać jedno konkretne pytanie,
4. jeśli są dane do sprawdzenia terminu — ustaw should_check_slots=true,
5. jeśli klient chce finalnie zapisać termin — ustaw should_create_booking=true,
6. nigdy nie obiecuj rezerwacji, jeśli backend nie potwierdzi dostępności,
7. nie twórz rezerwacji bez wyraźnego potwierdzenia klienta i wymaganych danych,
8. przed utworzeniem rezerwacji klient musi zaakceptować politykę prywatności,
9. zgoda marketingowa jest dobrowolna — zapytaj o nią osobno, ale nie blokuj rezerwacji, jeśli klient odmówi.

Dostępne usługi i pracownicy:
{json.dumps(service_brief, ensure_ascii=False)}
...
"""
    ...
    payload = {
        ...
        "text": {
            "format": {
                "type": "json_schema",
                "name": "alifio_ai_concierge_response",
                "strict": True,
                "schema": {
                    "type": "object",
                    ...
                    "properties": {
                        ...
                        "should_check_slots": {"type": "boolean"},
                        "should_create_booking": {"type": "boolean"}
                    },
                    "required": [
                        "reply",
                        "intent",
    ...
    try:
        ...
        output_text = ai_parse_openai_output(response_json)
        parsed = json.loads(output_text)

        return parsed

    except Exception as e:
        print("Błąd ai_call_openai:", e)

        return {
            "reply": "Przepraszam, asystent chwilowo nie może przetworzyć wiadomości. Spróbuj ponownie za moment.",
            "intent": "ai_error",
            "service_query": "",
            "employee_query": "",
            "date_iso": "",
...
def ai_create_booking_from_context(context):
    ...
    if not service_id or not employee_id or not booking_date or not booking_time:
        return {
            "success": False,
            "message": "Brakuje danych terminu: usługi, pracownika, daty lub godziny."
        }
    ...
    if not privacy_consent:
        return {
            "success": False,
            "message": "Przed zapisem rezerwacji potrzebuję potwierdzenia akceptacji polityki prywatności."
        }
    ...
    if booking_time not in available_slots:
        return {
            "success": False,
            "message": "Wybrany termin nie jest już dostępny. Mogę sprawdzić inne godziny."
        }
    ...
    if not client_id:
        return {
            "success": False,
            "message": "Nie udało się utworzyć lub odnaleźć karty klienta."
        }
    ...
    try:
        ...
        return {
            "success": False,
            "message": "Nie udało się zapisać rezerwacji. Spróbuj ponownie."
        }
    ...
    if client_email:
        try:
            ...
            print("Błąd wysyłki maila weryfikacyjnego AI booking:", e)
...
def ai_chat():
    ...
    if not message:
        return jsonify({
            "success": False,
            "reply": "Napisz wiadomość, a sprawdzę, jak mogę pomóc.",
            "slots": [],
            "suggestions": []
        }), 400
    ...
    if ai_text_confirms_privacy(message):
    ...
    ai_result = ai_call_openai(message, conversation, services)

    reply = ai_result.get("reply") or "Oczywiście. Proszę podać więcej szczegółów."
    slots = []
    suggestions = []

    service_query = ai_result.get("service_query") or ""
    employee_query = ai_result.get("employee_query") or ""
    date_iso = ai_result.get("date_iso") or ""
    time_from = ai_result.get("time_from") or ""
    time_to = ai_result.get("time_to") or ""
    booking_time = ai_result.get("booking_time") or ""

    if not booking_time:
        extracted_time = ai_extract_time_from_message(message)
        if extracted_time:
    ...
    if service_query:
        selected_service = ai_find_service(services, service_query)
    elif context.get("service_id"):
        selected_service = ai_find_service_by_id(services, context.get("service_id"))

    if selected_service:
        if employee_query:
            selected_employee = ai_find_employee(selected_service, employee_query)
        elif context.get("employee_id"):
            selected_employee = ai_find_employee_by_id(selected_service, context.get("employee_id"))

    if not selected_employee and selected_service:
    ...
    if booking_time:
        context["booking_time"] = booking_time

    if ai_result.get("client_name"):
        context["client_name"] = ai_result.get("client_name")

    if ai_result.get("client_phone"):
        context["client_phone"] = ai_result.get("client_phone")

    if ai_result.get("client_email"):
        context["client_email"] = ai_result.get("client_email")

    should_check_slots = ai_result.get("should_check_slots")

    if should_check_slots:
        if not selected_service:
            suggestions = [service["name"] for service in services[:6]]

            reply = (
                "Oczywiście. Jaką usługę chcesz zarezerwować? "
                "Mogę od razu sprawdzić dostępne terminy."
            )
            ...
            if len(employees) > 1:
                suggestions = [employee["name"] for employee in employees]

                reply = (
                    f"Dla usługi „{selected_service['name']}” dostępnych jest kilku specjalistów. "
                    "Którego pracownika wybierasz?"
                )
            else:
                reply = "Ta usługa nie ma obecnie przypisanego aktywnego pracownika."

        elif not context.get("date_iso"):
            reply = (
                f"Rozumiem. Chcesz zarezerwować „{selected_service['name']}”. "
                "Na jaki dzień mam sprawdzić dostępność?"
            )

        else:
            try:
                checked_date = context.get("date_iso")
                ...
                slots = ai_filter_slots_by_time(
                    available_slots,
                    time_from=context.get("time_from"),
                    time_to=context.get("time_to")
                )

                if slots:
                    slots_to_show = slots[:8]
                    slots_text = ", ".join(slots_to_show)

                    reply = (
                        f"Dla usługi „{selected_service['name']}” u {selected_employee['name']} "
                        f"w dniu {datetime.strptime(checked_date, '%Y-%m-%d').strftime('%d.%m.%Y')} "
                        f"mam dostępne godziny: {slots_text}. "
                        "Którą godzinę mam przygotować do rezerwacji?"
                    )

                    suggestions = slots_to_show
                else:
                    reply = (
                        f"Nie widzę wolnych terminów dla usługi „{selected_service['name']}” "
                        f"u {selected_employee['name']} w wybranym dniu. "
                        "Mogę sprawdzić inny dzień lub innego pracownika."
                    )

            except Exception as e:
                print("Błąd sprawdzania slotów AI:", e)
                reply = "Nie udało mi się poprawnie odczytać daty. Podaj ją proszę w formie: dzień, miesiąc, rok."

    has_core_booking_data = (
        context.get("service_id")
        and context.get("employee_id")
        and context.get("date_iso")
        and context.get("booking_time")
    )

    wants_booking = bool(ai_result.get("should_create_booking")) or bool(has_core_booking_data and booking_time)

    if wants_booking and has_core_booking_data:
        if not context.get("client_name"):
            reply = "Do przygotowania rezerwacji potrzebuję imienia i nazwiska. Jak mam zapisać klienta?"

        elif not context.get("client_phone"):
            reply = "Dziękuję. Potrzebuję jeszcze numeru telefonu do rezerwacji."

        elif not context.get("privacy_consent_confirmed"):
            privacy_url = ai_get_privacy_policy_url()

            if privacy_url:
                reply = (
                    "Aby zakończyć rezerwację, potrzebuję potwierdzenia akceptacji polityki prywatności.\n\n"
                    f"Polityka prywatności: {privacy_url}\n\n"
                    "Proszę napisać: „Akceptuję politykę prywatności”."
                )
            else:
                reply = (
                    "Aby zakończyć rezerwację, potrzebuję potwierdzenia akceptacji polityki prywatności.\n\n"
        ...
        elif not context.get("marketing_consent_answered"):
            reply = (
                "Dziękuję. Zgoda marketingowa jest dobrowolna.\n\n"
                "Czy chcesz otrzymywać informacje marketingowe i promocyjne? "
                "Możesz odpowiedzieć: „Tak” albo „Nie”."
            )
            ...
            if result.get("success"):
                service_name = context.get("service_name") or "wybrana usługa"
                employee_name = context.get("employee_name") or "wybrany pracownik"
                date_display = context.get("date_iso")
                ...
                reply = (
                    ...
                    "Dziękujemy. Jeśli podano adres e-mail, system wyśle wiadomość potwierdzającą."
                )
                ...
                reply = result.get("message") or "Nie udało się zapisać rezerwacji. Spróbuj ponownie."
