...
def ai_group_services_for_reply(services):
    ...
    for service in services:
        ...
        grouped[group_name].append(service)
...
def ai_services_reply(services):
    ...
    if not grouped:
        return (
            "Nie widzę obecnie aktywnych usług do rezerwacji. "
            "Proszę skontaktować się bezpośrednio z salonem."
        ), []

    lines = ["Oczywiście. Wybierz grupę lub konkretną usługę:"]
    ...
    for group_name, group_services in grouped.items():
        ...
        for service in group_services:
            label = f"{service['name']} — {service.get('price') or 'cena do potwierdzenia'}"
...
def ai_employees_reply(service):
    ...
    if not employees:
        return (
            f"Usługa „{service['name']}” nie ma obecnie przypisanego aktywnego pracownika.",
            []
        )
    ...
    if len(employees) == 1:
        ...
        return (
            f"Dla usługi „{service['name']}” dostępny jest pracownik: {employee['name']}. "
            "Na jaki dzień mam sprawdzić wolne godziny?",
            [employee["name"]]
        )

    lines = [
        f"Dla usługi „{service['name']}” możesz wybrać pracownika:"
    ]
...
def ai_format_slots_reply(service, employee, date_iso, slots):
    ...
    if not slots:
        return (
            ...
            "Możesz podać inny dzień albo wybrać innego pracownika."
        ), []
    ...
    return (
        ...
        + "\n".join([f"• {slot}" for slot in slots_to_show])
...
def ai_chat():
    ...
    if selected_service and context.get("employee_id"):
    ...
    service_query = ai_result.get("service_query") or message
    employee_query = ai_result.get("employee_query") or message
    date_iso = ai_result.get("date_iso") or ai_extract_date_from_message(message)
    booking_time = ai_result.get("booking_time") or ai_extract_time_from_message(message)

    client_email = ai_result.get("client_email") or ai_extract_email_from_message(message)
    client_phone = ai_result.get("client_phone") or ai_extract_phone_from_message(message)
    client_name = (ai_result.get("client_name") or "").strip()

    if not selected_service:
        found_service = ai_find_service(services, service_query)
    ...
    if selected_service and not selected_employee:
        found_employee = ai_find_employee(selected_service, employee_query)
    ...
    if client_name and len(client_name.split()) >= 2:
    ...
    if not context.get("date_iso"):
        reply = (
            ...
            "Na jaki dzień mam sprawdzić dostępne godziny?"
        )
    ...
    try:
        ...
        reply = "Nie udało mi się poprawnie odczytać daty. Podaj ją proszę w formacie DD.MM.RRRR, np. 12.05.2026."
    ...
    if context.get("booking_time") not in slots:
        context.pop("booking_time", None)
        ...
        reply = (
            "Ta godzina nie jest dostępna albo została już zajęta.\n\n"
            + reply
        )
    ...
    if not context.get("client_name"):
        reply = "Świetnie. Podaj proszę imię i nazwisko do rezerwacji."
    ...
    if not context.get("client_email"):
        reply = (
            ...
            "Jeśli klient nie chce podawać adresu e-mail, napisz: „bez maila”."
        )

        if "bez maila" in ai_normalize_text(message) or "bez emaila" in ai_normalize_text(message):
    ...
    if not context.get("privacy_consent_confirmed"):
        ...
        if privacy_url:
            reply = (
                ...
                "Napisz proszę: „Akceptuję politykę prywatności”."
            )
        else:
            reply = (
                "Aby zapisać rezerwację, potrzebuję potwierdzenia akceptacji polityki prywatności.\n\n"
