...
def build_internal_email_wrapper(
    ...
    safe_company_name = escape(company_name or "Salon")
...
def build_booking_internal_email_html(payload, recipient_label: str) -> str:
    ...
    rows_html = "".join([
        build_email_detail_row("Usluga:", payload["service_name"] or "—"),
        ...
        build_email_detail_row("Data:", format_date_pl(payload["booking_date"])),
        build_email_detail_row("Godzina:", payload["booking_time"] or "—"),
        build_email_detail_row("Notatki:", (payload["notes"] or "").strip() or "Brak notatki."),
    ])

    return build_internal_email_wrapper(
        company_name=company_name,
        intro_text="W systemie zostala wlasnie zapisana nowa rezerwacja.",
...
def build_booking_internal_email_text(payload, recipient_label: str) -> str:
    return (
        ...
        f"Usluga: {payload['service_name'] or '—'}\n"
...
def build_waitlist_internal_email_html(payload, recipient_label: str) -> str:
    ...
    return build_internal_email_wrapper(
        company_name=company_name,
        intro_text="W systemie pojawil sie nowy wpis na liscie oczekujacych.",
        recipient_label=recipient_label,
        rows_html=rows_html
    )
...
def build_waitlist_internal_email_text(payload, recipient_label: str) -> str:
    return (
        f"Nowy wpis na liscie oczekujacych w systemie {payload['company_name'] or 'Salon'}\n\n"
        f"Odbiorca powiadomienia: {recipient_label}\n\n"
        f"Usluga: {payload['service_name'] or '—'}\n"
        f"Specjalista: {payload['employee_name'] or '—'}\n"
        f"Klient: {payload['client_name'] or '—'}\n"
...
def send_waitlist_internal_notifications(waitlist_id: int) -> dict:
    ...
    if salon_email:
        result["salon_sent"] = send_email_message(
            salon_email,
            f"Nowa lista oczekujacych — {company_name}",
...
def send_booking_cancellation_confirmation_email(booking_data: dict) -> bool:
    ...
    text_body = (
        ...
        f"Usluga: {(booking_data.get('service_name') or '—').strip()}\n"
...
def send_booking_cancellation_internal_notifications(booking_data: dict) -> bool:
    ...
    subject = f"Anulowano rezerwacje — {client_name}"
