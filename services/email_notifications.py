...
def send_email_smtp(to_email, subject, body):
    """
    Wrapper zostawiony po to, żeby app.py nie wysypał się na imporcie.
    Docelowo najlepiej w app.py zamienić użycia send_email_smtp na send_email_message.
    """
    text_body = (body or "").strip()
    html_body = f"<pre style=\"font-family:Arial, sans-serif; white-space:pre-wrap; line-height:1.6;\">{escape(text_body)}</pre>"
