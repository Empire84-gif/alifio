...
def get_company_team_signature(value: str | None) -> str:
    return f"Zespół {get_company_display_name(value)}"
...
def send_booking_verification_email(booking_id: int, cancel_url: str | None = None) -> bool:
    ...
    subject = "Potwierdź adres e-mail i dokończ aktywację komunikacji"
    ...
    text_body = (
        f"Dziękujemy za dokonanie rezerwacji w systemie {company_name}.\n\n"
        "Aby otrzymywać kolejne wiadomości dotyczące wizyty, potwierdź swój adres e-mail, korzystając z poniższego linku:\n"
        f"{verification_link}"
    )
...
def send_waitlist_verification_email(waitlist_id: int) -> bool:
    ...
    subject = "Potwierdź adres e-mail i aktywuj powiadomienia dotyczące listy oczekujących"
    ...
    text_body = (
        f"Twój zapis na listę oczekujących w systemie {company_name} został przyjęty.\n\n"
        "Aby otrzymywać kolejne wiadomości dotyczące zgłoszenia, potwierdź swój adres e-mail, korzystając z poniższego linku:\n"
        f"{verification_link}\n\n"
        f"Pozdrawiamy,\n{team_signature}"
    )
...
def build_internal_email_wrapper(
    ...
    team_signature = get_company_team_signature(company_name)
    ...
    safe_team_signature = escape(team_signature)
      ...
      <div style="max-width:680px; margin:0 auto; padding:32px 18px;">
        <div style="background:#ffffff; border:1px solid #e7ddd4; box-shadow:0 10px 30px rgba(30,22,17,0.06);">
          ...
          <div style="padding:18px 30px 24px 30px; border-top:1px solid #efe5dc; background:#fcfaf8; font-size:14px; line-height:1.8; color:#5d5149;">
            Pozdrawiamy,<br>
            <strong>{safe_team_signature}</strong>
          </div>
        </div>
      </div>
    </div>
...
def build_booking_internal_email_html(payload, recipient_label: str) -> str:
    company_name = payload["company_name"] or "Salon"

    rows_html = "".join([
        build_email_detail_row("Usługa:", payload["service_name"] or "—"),
        build_email_detail_row("Specjalista:", payload["employee_name"] or "—"),
...
def send_waitlist_internal_notifications(waitlist_id: int) -> dict:
    ...
    if salon_email:
        result["salon_sent"] = send_email_message(
            salon_email,
            f"Nowy wpis na liście oczekujących — {company_name}",
...
def send_booking_cancellation_internal_notifications(booking_data: dict) -> bool:
    ...
    text_body = (
        "W systemie anulowano rezerwację.\n\n"
