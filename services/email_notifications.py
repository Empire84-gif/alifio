...
DEFAULT_FROM_EMAIL = "office@sddestonie.com"
...
def build_unsubscribe_booking_emails_link(token: str) -> str:
    return url_for("unsubscribe_booking_emails_page", token=token, _external=True)


def build_withdraw_marketing_consent_link(token: str) -> str:
    return url_for("withdraw_marketing_consent_page", token=token, _external=True)
...
def build_company_email_context(payload: dict) -> dict:
    ...
    return {
        ...
        "privacy_policy_url": (payload.get("privacy_policy_url") or "").strip(),
        "unsubscribe_booking_emails_url": (payload.get("unsubscribe_booking_emails_url") or "").strip(),
        "withdraw_marketing_consent_url": (payload.get("withdraw_marketing_consent_url") or "").strip(),
    }
...
def build_company_footer_html(payload: dict) -> str:
    ...
    if company["website_url"]:
        ...
        rows.append(
            f'Strona: <a href="{safe_url}" style="color:#7C6B5F; text-decoration:none;">{safe_url}</a>'
        )

    if company["privacy_policy_url"]:
        safe_url = escape(company["privacy_policy_url"])
        rows.append(
            f'<a href="{safe_url}" style="color:#7C6B5F; text-decoration:none;">Polityka prywatności</a>'
        )

    if company["unsubscribe_booking_emails_url"]:
        safe_url = escape(company["unsubscribe_booking_emails_url"])
        rows.append(
            f'<a href="{safe_url}" style="color:#7C6B5F; text-decoration:none;">Zrezygnuj z powiadomień e-mail o rezerwacjach</a>'
        )

    if company["withdraw_marketing_consent_url"]:
        safe_url = escape(company["withdraw_marketing_consent_url"])
        rows.append(
            f'<a href="{safe_url}" style="color:#7C6B5F; text-decoration:none;">Wycofaj zgodę marketingową</a>'
        )
...
def build_company_footer_text(payload: dict) -> str:
    ...
    if company["privacy_policy_url"]:
        rows.append(f"Polityka prywatności: {company['privacy_policy_url']}")

    if company["unsubscribe_booking_emails_url"]:
        rows.append(f"Zrezygnuj z powiadomień e-mail o rezerwacjach: {company['unsubscribe_booking_emails_url']}")

    if company["withdraw_marketing_consent_url"]:
        rows.append(f"Wycofaj zgodę marketingową: {company['withdraw_marketing_consent_url']}")
...
def send_email_message(
    ...
    smtp_use_tls = bool(current_app.config.get("MAIL_SMTP_USE_TLS", False))
    smtp_use_ssl = bool(current_app.config.get("MAIL_SMTP_USE_SSL", False))
    from_email = current_app.config.get("MAIL_FROM_EMAIL", DEFAULT_FROM_EMAIL)
    reply_to = current_app.config.get("MAIL_REPLY_TO_EMAIL")
    ...
    if not smtp_username:
        missing.append("MAIL_SMTP_USERNAME")
    if not smtp_password:
        missing.append("MAIL_SMTP_PASSWORD")
    if not from_email:
        missing.append("MAIL_FROM_EMAIL")

    if missing:
        raise RuntimeError(f"Brak konfiguracji maili: {', '.join(missing)}")
    ...
    current_app.logger.info("[MAIL] Wysyłam maila do: %s | Temat: %s", to_email, subject)
    ...
    try:
        ...
        current_app.logger.info("[MAIL] Wysłano maila do: %s", to_email)
        return True

    except Exception as exc:
        current_app.logger.exception("[MAIL] Błąd wysyłki do %s: %s", to_email, exc)
        raise
...
def send_email_to_many(
    ...
    for recipient in clean_recipients:
        try:
            ...
            current_app.logger.exception("[MAIL] Nie udało się wysłać do %s: %s", recipient, exc)
...
def send_email_smtp(
    ...
    if html_body is not None:
        ...
        final_text_body = text_body or ""
    else:
        final_text_body = (body or "").strip()
        final_html_body = (
            '<pre style="font-family:Arial, sans-serif; white-space:pre-wrap; line-height:1.6;">'
            f"{escape(final_text_body)}"
            "</pre>"
        )
...
def get_or_assign_client_action_token(client_id: int | None) -> str | None:
    if not client_id:
        return None
    ...
    try:
        ...
        if client and client["email_verification_token"]:
            return client["email_verification_token"]

    finally:
        conn.close()

    return assign_client_verification_token(client_id)


def attach_client_action_links(payload: dict, token: str | None) -> dict:
    payload = row_to_dict(payload)

    if not token:
        return payload

    payload["unsubscribe_booking_emails_url"] = build_unsubscribe_booking_emails_link(token)
    payload["withdraw_marketing_consent_url"] = build_withdraw_marketing_consent_link(token)
...
def send_booking_verification_email(booking_id: int, cancel_url: str | None = None) -> bool:
    ...
    payload = attach_client_action_links(payload, token)
    ...
    html_body = render_template(
        ...
        email_title=subject,
        email_heading="Potwierdzenie adresu email",
        **company_ctx,
    )
...
def send_waitlist_verification_email(waitlist_id: int) -> bool:
    ...
    cancel_token = (payload.get("cancel_token") or "").strip()
    cancel_url = ""

    if cancel_token:
        cancel_url = url_for(
            "cancel_waitlist_entry_from_link",
            token=cancel_token,
            _external=True
        )
    ...
    if cancel_url:
        text_body += (
            "Jeżeli chcesz anulować wpis na listę oczekujących, możesz skorzystać z poniższego linku:\n\n"
            f"{cancel_url}\n\n"
        )
...
def build_booking_internal_email_html(payload, recipient_label: str) -> str:
    ...
    rows_html = "".join([
        ...
        build_email_detail_row("Zgoda na politykę prywatności:", format_consent_status(payload.get("privacy_consent"))),
        build_email_detail_row("Zgoda marketingowa:", format_consent_status(payload.get("marketing_consent"))),
        ...
        build_email_detail_row("Data udzielenia zgód:", payload.get("consents_created_at") or "—"),
    ])
...
def send_booking_cancellation_confirmation_email(booking_data: dict) -> bool:
    ...
    token = get_or_assign_client_action_token(booking_data.get("client_id"))
    booking_data = attach_client_action_links(booking_data, token)
...
def send_booking_cancellation_internal_notifications(booking_data: dict) -> bool:
    ...
    html_body = render_template(
        ...
        data={
            ...
            "privacy_policy_url": company_ctx["privacy_policy_url"],
